  ...
  const csvLines = [
    ...
    ...rows.map((row) =>
      [
        ...
        row.category, row.subcategory, row.allSubcategories, row.website,
        row.phone, row.address, row.reviewCount, row.reviewRating,
        row.status, row.priceRange, row.foursquareId, row.foursquareUrl,
      ].map(escapeCsv).join(",")
    ),
  ];
