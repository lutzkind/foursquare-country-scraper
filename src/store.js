  ...
  return {
    ...
    createSession({ id, username, expiresAt }) {
      ...
      db.prepare(
        ...
        `
      ).run({ id, username, expiresAt, timestamp });
    },
    ...
    touchSession(sessionId, expiresAt) {
      const timestamp = nowIso();
      db.prepare(
        `UPDATE sessions SET expires_at = @expiresAt, last_seen_at = @timestamp WHERE id = @id`
      ).run({ id: sessionId, expiresAt, timestamp });
    },
    ...
    setAppSettings(settings) {
      ...
      db.transaction(() => {
        for (const [key, value] of Object.entries(settings)) {
          upsert.run({ key, valueJson: JSON.stringify(value), timestamp });
        }
      })();
    },
    ...
    getNocoDbSyncState(jobId) {
      const row = db
        .prepare(`SELECT * FROM nocodb_sync_state WHERE job_id = ?`)
        .get(jobId);
    ...
    markNocoDbSyncStarted(jobId) {
      ...
      db.prepare(
        `
          ...
          ) VALUES (@jobId, 'running', 'Sync in progress.', @timestamp, NULL)
    ...
    markNocoDbSyncFailure(jobId, message) {
      ...
      db.prepare(
        `
          INSERT INTO nocodb_sync_state (
            job_id, last_status, last_message, last_started_at, last_finished_at
          ) VALUES (@jobId, 'failed', @message, @timestamp, @timestamp)
          ON CONFLICT(job_id) DO UPDATE SET
            last_status = 'failed',
            last_message = @message,
            last_finished_at = @timestamp
    ...
    cancelJob(jobId) {
      ...
      db.transaction(() => {
        db.prepare(
          `UPDATE jobs SET status = 'canceled', message = 'Canceled', finished_at = @timestamp, updated_at = @timestamp WHERE id = @jobId`
        ).run({ jobId, timestamp });
        db.prepare(
          `UPDATE shards SET status = 'canceled', updated_at = @timestamp WHERE job_id = @jobId AND status IN ('pending', 'retry', 'running')`
        ).run({ jobId, timestamp });
      })();
      this.refreshJobStats(jobId);
    ...
    resumeJob(jobId) {
      const job = this.getJob(jobId);
      if (!job) return null;
      const resumedStatus = job.totalShards > 0 || job.startedAt ? "running" : "pending";
      const message = resumedStatus === "running" ? "Running" : "Queued";
      db.prepare(
        `UPDATE jobs SET status = @status, message = @message, updated_at = @timestamp WHERE id = @jobId`
      ).run({ jobId, status: resumedStatus, message, timestamp: nowIso() });
      return this.getJob(jobId);
    },
    ...
    deleteJob(jobId) {
      ...
      if (!["completed", "partial", "failed", "canceled"].includes(job.status)) {
        const error = new Error("Only completed, partial, failed, or canceled jobs can be deleted.");
      ...
      for (const artifactPath of artifactPaths) {
        try { fs.unlinkSync(artifactPath); } catch (error) {
    ...
    getJobLeadsAfterId(jobId, leadId = 0, { limit = 100 } = {}) {
      return db
        .prepare(`SELECT * FROM leads WHERE job_id = ? AND id > ? ORDER BY id ASC LIMIT ?`)
    ...
    countJobLeadsAfterId(jobId, leadId = 0) {
      const row = db
        .prepare(`SELECT COUNT(*) AS total FROM leads WHERE job_id = ? AND id > ?`)
        .get(jobId, leadId);
      return row?.total || 0;
    },
    ...
    claimNextShard() {
      ...
      db.prepare(
        `UPDATE shards SET status = 'running', attempt_count = attempt_count + 1, updated_at = @timestamp, run_token = @runToken WHERE id = @id`
    ...
    getShard(shardId) {
      const row = db.prepare(`SELECT * FROM shards WHERE id = ?`).get(shardId);
    ...
    skipShard(shardId, message, runToken = null) {
      ...
      const result = db.prepare(
        `UPDATE shards SET status = 'skipped', run_token = NULL, last_error = @message, updated_at = @timestamp WHERE id = @id ${buildOwnedRunningShardClause(runToken)}`
      ).run(buildOwnedRunningShardParams({ id: shardId, message, timestamp: nowIso(), runToken }));
    ...
    completeShard(shardId, leads, runToken = null) {
      ...
      const completedJobId = db.transaction(() => {
        const ownedShard = db
          .prepare(`SELECT id FROM shards WHERE id = @id ${buildOwnedRunningShardClause(runToken)}`)
          .get(buildOwnedRunningShardParams({ id: shardId, runToken }));
        if (!ownedShard) return null;
        ...
        const insert = db.prepare(
          `
            ...
            ON CONFLICT(job_id, dedupe_key) DO UPDATE SET
              ...
              website = CASE WHEN COALESCE(leads.website, '') = '' THEN excluded.website ELSE leads.website END,
              phone = CASE WHEN COALESCE(leads.phone, '') = '' THEN excluded.phone ELSE leads.phone END,
              address = CASE WHEN COALESCE(leads.address, '') = '' THEN excluded.address ELSE leads.address END,
              complete_address_json = excluded.complete_address_json,
              city = CASE WHEN COALESCE(leads.city, '') = '' THEN excluded.city ELSE leads.city END,
              area = CASE WHEN COALESCE(leads.area, '') = '' THEN excluded.area ELSE leads.area END,
              state_region = CASE WHEN COALESCE(leads.state_region, '') = '' THEN excluded.state_region ELSE leads.state_region END,
              postcode = CASE WHEN COALESCE(leads.postcode, '') = '' THEN excluded.postcode ELSE leads.postcode END,
              country = CASE WHEN COALESCE(leads.country, '') = '' THEN excluded.country ELSE leads.country END,
              review_count = CASE WHEN excluded.review_count > COALESCE(leads.review_count, 0) THEN excluded.review_count ELSE leads.review_count END,
        ...
        db.prepare(
          `UPDATE shards SET status = 'done', result_count = @resultCount, run_token = NULL, last_error = NULL, updated_at = @timestamp WHERE id = @id ${buildOwnedRunningShardClause(runToken)}`
        ).run(buildOwnedRunningShardParams({ id: shardId, resultCount: leads.length, timestamp, runToken }));
    ...
    retryShard(shardId, errorMessage, delayMs, runToken = null) {
      ...
      const result = db.prepare(
        `UPDATE shards SET status = 'retry', run_token = NULL, last_error = @errorMessage, next_run_at = @nextRunAt, updated_at = @timestamp WHERE id = @id ${buildOwnedRunningShardClause(runToken)}`
      ).run(buildOwnedRunningShardParams({ id: shardId, errorMessage, nextRunAt, timestamp: nowIso(), runToken }));
      if (result.changes === 0) return null;
      this.refreshJobStats(shard.jobId);
      return shard.jobId;
    },
    ...
    failShard(shardId, errorMessage, runToken = null) {
      ...
      if (!shard) return null;
      const result = db.prepare(
        `UPDATE shards SET status = 'failed', run_token = NULL, last_error = @errorMessage, updated_at = @timestamp WHERE id = @id ${buildOwnedRunningShardClause(runToken)}`
      ).run(buildOwnedRunningShardParams({ id: shardId, errorMessage, timestamp: nowIso(), runToken }));
      if (result.changes === 0) return null;
      this.refreshJobStats(shard.jobId);
      return shard.jobId;
    },
    ...
    reclaimStaleRunningShards(staleMs) {
      ...
      const timestamp = nowIso();
      const staleJobs = db
        .prepare(`SELECT DISTINCT job_id AS jobId FROM shards WHERE status = 'running' AND updated_at <= @staleBefore`)
        .all({ staleBefore })
        .map((row) => row.jobId);
      if (staleJobs.length === 0) return [];
      db.prepare(
        `UPDATE shards SET status = 'retry', run_token = NULL, next_run_at = @timestamp, updated_at = @timestamp, last_error = CASE WHEN COALESCE(last_error, '') = '' THEN @message ELSE last_error END WHERE status = 'running' AND updated_at <= @staleBefore`
      ).run({ staleBefore, timestamp, message: "Recovered after running shard exceeded the stale timeout." });
      for (const jobId of staleJobs) this.refreshJobStats(jobId);
      return staleJobs;
    },
    ...
    refreshJobStats(jobId) {
      const shardStats = db
        .prepare(
          `SELECT COUNT(*) AS total_shards, SUM(CASE WHEN status = 'done' THEN 1 ELSE 0 END) AS completed_shards, SUM(CASE WHEN status = 'failed' THEN 1 ELSE 0 END) AS failed_shards, SUM(CASE WHEN status IN ('pending', 'retry', 'running') THEN 1 ELSE 0 END) AS unfinished_shards FROM shards WHERE job_id = ?`
        )
        .get(jobId);
      const leadStats = db.prepare(`SELECT COUNT(*) AS lead_count FROM leads WHERE job_id = ?`).get(jobId);
      db.prepare(
        `UPDATE jobs SET total_shards = @totalShards, completed_shards = @completedShards, failed_shards = @failedShards, lead_count = @leadCount, updated_at = @timestamp WHERE id = @jobId`
      ).run({
        jobId,
    ...
    finalizeJob(jobId, status, message, artifacts = {}) {
      db.prepare(
        `UPDATE jobs SET status = @status, message = @message, artifact_csv_path = COALESCE(@csvPath, artifact_csv_path), artifact_json_path = COALESCE(@jsonPath, artifact_json_path), finished_at = @timestamp, updated_at = @timestamp WHERE id = @jobId`
      ).run({
...
function resetRunningShards(db) {
  const timestamp = nowIso();
  db.prepare(
    `UPDATE shards SET status = 'retry', run_token = NULL, next_run_at = @timestamp, updated_at = @timestamp, last_error = COALESCE(last_error, 'Recovered after process restart.') WHERE status = 'running'`
  ).run({ timestamp });
  db.prepare(
    `UPDATE jobs SET status = CASE WHEN status = 'running' THEN 'pending' ELSE status END, message = CASE WHEN status = 'running' THEN 'Recovered after process restart.' ELSE message END, updated_at = @timestamp WHERE status = 'running'`
  ).run({ timestamp });
}

function cleanupExpiredSessions(db) {
  db.prepare(`DELETE FROM sessions WHERE expires_at <= @timestamp`).run({ timestamp: nowIso() });
}

function buildOwnedRunningShardClause(runToken) {
  return runToken ? "AND status = 'running' AND run_token = @runToken" : "";
}
...
function backfillLeadLocations(db) {
  const rows = db
    .prepare(
      `SELECT id, raw_json, complete_address_json, city, area, state_region, postcode, country FROM leads WHERE COALESCE(city, '') = '' OR COALESCE(area, '') = '' OR COALESCE(state_region, '') = '' OR COALESCE(postcode, '') = '' OR COALESCE(country, '') = ''`
    )
    .all();
  if (!rows.length) return;
  const update = db.prepare(
    `UPDATE leads SET city = CASE WHEN COALESCE(city, '') = '' THEN @city ELSE city END, area = CASE WHEN COALESCE(area, '') = '' THEN @area ELSE area END, state_region = CASE WHEN COALESCE(state_region, '') = '' THEN @stateRegion ELSE state_region END, postcode = CASE WHEN COALESCE(postcode, '') = '' THEN @postcode ELSE postcode END, country = CASE WHEN COALESCE(country, '') = '' THEN @country ELSE country END WHERE id = @id`
  );
  db.transaction(() => {
    for (const row of rows) {
      const raw = safeJsonParse(row.raw_json);
      const completeAddress = safeJsonParse(row.complete_address_json);
      const location = extractLocationParts(raw || { location: completeAddress });
      update.run({ id: row.id, city: location.city, area: location.area, stateRegion: location.stateRegion, postcode: location.postcode, country: location.country });
    }
  })();
}
...
function deserializeJobRow(row) {
  return {
    ...
    countryGeometry: row.country_geometry_json ? JSON.parse(row.country_geometry_json) : null,
...
function deserializeLeadRow(row) {
  ...
  const allSubcategories = extractFoursquareSubcategories(row.category, categories);
  return {
    id: row.id,
    jobId: row.job_id,
...
function extractFoursquareSubcategories(primaryCategory, categories) {
...
function sanitizeNocoDbConfig(input) {
  const autoSyncIntervalMinutes = Number.parseInt(String(input.autoSyncIntervalMinutes ?? "0"), 10);
  return {
    ...
    autoSyncIntervalMinutes:
      Number.isFinite(autoSyncIntervalMinutes) && autoSyncIntervalMinutes > 0
        ? autoSyncIntervalMinutes
        : 0,
    autoCreateColumns: input.autoCreateColumns == null ? true : Boolean(input.autoCreateColumns),
  };
}
