const STANDARD_FIELDS = [
  ...
  { name: "foursquare_id", type: "SingleLineText" },
  { name: "foursquare_url", type: "URL" },
...
function createNocoDbService({ store, config }) {
  return {
    ...
    async syncJob(jobId, options = {}) {
      ...
      try {
        ...
        if (settings.autoCreateColumns) {
          const missingFields = desiredFields.filter((field) => !availableFields.has(field.name));
          for (const field of missingFields) await createColumn(settings, field);
          if (missingFields.length > 0) {
            columns = await listColumns(settings);
            availableFields = collectColumnNames(columns);
          }
        }
        const syncState = options.force ? defaultSyncState(jobId) : store.getNocoDbSyncState(jobId);
        let lastSyncedLeadId = options.force ? 0 : syncState.lastSyncedLeadId;
        let syncedRecordCount = 0;
        while (true) {
          const leads = store.getJobLeadsAfterId(jobId, lastSyncedLeadId, { limit: 100 });
        ...
        store.markNocoDbSyncSuccess(jobId, { lastSyncedLeadId, syncedRecordCount, message });
        return {
          ok: true, jobId, syncedRecordCount,
          config: toPublicConfig(settings),
    ...
    async syncCompletedJobIfEnabled(jobId) {
      const settings = store.getNocoDbConfig();
      if (!settings.autoSyncOnCompletion || !hasEnoughSettings(settings)) return null;
      try {
    ...
    getRunningJobSyncIdsDue() {
      const settings = store.getNocoDbConfig();
      if (!hasEnoughSettings(settings)) return [];
      ...
      return store
        ...
        .filter((job) => {
          ...
          if (syncState.lastStatus === "running") return false;
          if (!store.countJobLeadsAfterId(job.id, syncState.lastSyncedLeadId || 0)) return false;
...
function resolveSettings(store, input) {
  if (!input) return store.getNocoDbConfig();
  const current = store.getNocoDbConfig();
  return sanitizeSettings({
    ...current, ...input,
    apiToken: input.apiToken == null || input.apiToken === "" ? current.apiToken : input.apiToken,
  });
}
...
function validateSettings(settings) {
  if (!hasEnoughSettings(settings)) {
    throw createHttpError(400, "NocoDB base URL, API token, base ID, and table ID are required.");
  }
}

function hasEnoughSettings(settings) {
  return Boolean(settings.baseUrl && settings.apiToken && settings.baseId && settings.tableId);
}
...
function buildSyncTelemetry(store, settings, jobId, sync = null) {
  ...
  const job = store.getJob(jobId);
  const unsyncedLeadCount = store.countJobLeadsAfterId(jobId, syncState.lastSyncedLeadId || 0);
  return {
    unsyncedLeadCount,
...
function getNextIncrementalSyncAt(job, syncState, settings, unsyncedLeadCount = null) {
  ...
  const intervalMinutes = Number(settings.autoSyncIntervalMinutes || 0);
  if (!intervalMinutes || syncState.lastStatus === "running") return null;
  const pendingCount = unsyncedLeadCount == null ? 0 : Number.parseInt(String(unsyncedLeadCount), 10);
  ...
  if (!referenceTime) return new Date().toISOString();
...
function buildDesiredFields() {
  ...
  for (const field of STANDARD_FIELDS) byName.set(field.name, field);
...
function buildRecord(job, lead, availableFields) {
  const record = {
    ...
    foursquare_id: lead.placeId || "",
    foursquare_url: lead.link || "",
    name: lead.name || "",
    category: lead.category || "",
    subcategory: lead.subcategory || "",
    all_subcategories: Array.isArray(lead.allSubcategories) ? lead.allSubcategories.join(" | ") : "",
...
async function createColumn(settings, field) {
  const payload = { title: field.name, column_name: field.name, name: field.name, uidt: field.type, type: field.type };
  return apiRequestFallback(settings, [
    {
      pathname: `/api/v2/base/${encodeURIComponent(settings.baseId)}/table/${encodeURIComponent(settings.tableId)}/column`,
...
async function apiRequestFallback(settings, attempts) {
  ...
  for (const attempt of attempts) {
    try {
      ...
      return typeof attempt.transform === "function" ? attempt.transform(result) : result;
    } catch (error) {
      lastError = error;
...
function collectColumnNames(columns) {
  ...
  for (const column of columns) {
    for (const value of [column.column_name, column.name, column.title, column.displayName]) {
...
function defaultSyncState(jobId) {
  return { jobId, lastSyncedLeadId: 0, lastSyncedAt: null, lastStatus: "idle", lastMessage: null, lastStartedAt: null, lastFinishedAt: null, syncedRecordCount: 0 };
}
